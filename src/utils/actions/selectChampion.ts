import { Data, Hero, DraftAction } from '@/types/global';
...
const getHeroFromPool = (heroesPool: Hero[], heroName: string | null): Hero | null => {
  if (!heroName) return null;
  const hero = heroesPool.find(h => h && h.id === heroName);
  return hero?.selected ? null : (hero ?? null);
};

const updateHeroSelectionInPool = (heroesPool: Hero[], hero: Hero | null): Hero[] =>
  heroesPool.map(h => {
...
// Main Function
export const selectChampion = async (data: Data, action: DraftAction): Promise<void> => {
  try {
...
export {
  getRandomUnselectedHero,
  getHeroFromPool,
  updateHeroSelectionInPool,
  updateTeamHeroSelection,
};