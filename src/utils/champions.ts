...
const getHeroFromPool = (heroes_pool: Hero[], heroName: string | null) => {
  const hero = heroName ? heroes_pool.find(h => h.name === heroName) : undefined;
  if (hero?.selected) throw new Error("Hero already selected");
  return hero;
};

const updateHeroSelectionInPool = (heroes_pool: Hero[], hero?: Hero) =>
  heroes_pool.map(h => (h.name === hero?.name ? { ...h, selected: true } : h));

const updateTeamHeroSelection = (heroes: Hero[], hero?: Hero) => {
  const index = heroes.findIndex(h => !h.selected);
  if (index !== -1) heroes[index] = { id: hero?.id || '', name: hero?.name || null, selected: true };
};

async function selectChampion(roomid: string, selectedChampion: string | null) {
  const teamResponse = await supabase.from('teams').select('id, isturn, heroes_selected, heroes_ban, clicked_hero, nb_turn').eq('room', roomid).eq('isturn', true).single();
  const roomResponse = await supabase.from('rooms').select('id, status, heroes_pool').eq('id', roomid).single();
  ...
  const heroesAction = action === 'ban' ? team.heroes_ban : team.heroes_selected;
  ...
  const hero = selectedChampion ? getHeroFromPool(room.heroes_pool, selectedChampion) : (action === 'select' ? getRandomUnselectedHero(room.heroes_pool) : undefined);
  ...
  await supabase.from('teams').update({ [actionFunctionMapping[action]]: heroesAction, nb_turn: team.nb_turn - 1 }).eq('id', team.id);

  // Update room in the database
  await supabase.from('rooms').update({ heroes_pool: updatedHeroesPool }).eq('id', roomid);
}
