...
export class RoomTimerManager {
  ...
  private addTimerEventListeners(timer: Timer, roomid: string, io: Server, onTargetAchieved?: () => Promise<void>) {
    let lastSecond = -1;

    timer.addEventListener('secondsUpdated', () => {
      const timeValues = timer.getTimeValues();
      const currentSecond = timeValues.seconds;
      console.log("timer.addEventListener - currentSecond:", currentSecond);
  ...
  startLobbyTimer(roomid: string) {
    if (!this.roomTimers[roomid]) return;
    this.roomTimers[roomid].countdownTimerLobby.start({ countdown: true, startValues: { seconds: Number(process.env.LOBBY_TIME) || 20 } });
  }

  startTimer(roomid: string) {
    if (!this.roomTimers[roomid]) return;
    this.roomTimers[roomid].countdownTimer.start({ countdown: true, startValues: { seconds: Number(process.env.START_TIME) || 15 } });
  }
